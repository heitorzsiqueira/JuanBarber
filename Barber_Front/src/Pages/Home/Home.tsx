...
export default function Home() {
  ...
  useEffect(() => {
    async function loadUserData() {
      try {
        // Usando a rota de sessão para pegar os dados reais do banco
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xl font-bold tracking-tighter uppercase">Juan Barber</span>
            </div>

            <div className="flex items-center gap-4">
              <div className="hidden sm:flex flex-col items-end text-sm">
                <span className="font-medium text-zinc-100">{user?.name}</span>
                <span className="text-zinc-500 text-xs capitalize">{user?.role}</span>
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          >
            ...
            <p className="text-zinc-500">
              {user?.role === 'CLIENT'
                ? 'Veja seus horários marcados e histórico de cortes.'
                : 'Gerencie sua agenda e veja os próximos clientes.'}
            </p>
          </div>
          ...
          >
            ...
            <h3 className="text-xl font-bold mb-2">
              {user?.role === 'JUAN' ? 'Gerenciar Serviços' : 'Serviços & Preços'}
            </h3>

            <p className="text-zinc-500">
              {user?.role === 'JUAN'
                ? 'Edite valores, nomes e descrições dos serviços.'
                : 'Cabelo, barba, sobrancelha e combos.'}
            </p>
          </div>
